...
import { X, Phone, MessageCircle, MapPin, CalendarCheck, ArrowRight } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useActiveAds, useRecordImpression, recordAdClick, type ActiveAd } from "@/hooks/useActiveAds";
...
const BannerAdSlot = ({ variant = "inline", adType }: BannerAdSlotProps) => {
  ...
  const handleCta = (action: string) => {
    ...
    switch (action) {
      case "call":
        ...
        break;
      case "chat":
        navigate(`/messaging?businessId=${businessCard?.id || ad?.business_card_id}`);
        break;
      case "directions":
        if (businessCard?.maps_link) {
          window.open(businessCard.maps_link);
        } else if (businessCard?.latitude && businessCard?.longitude) {
          window.open(`https://www.google.com/maps?q=${businessCard.latitude},${businessCard.longitude}`);
        }
        break;
      case "book":
  ...
  const hasImage = ad?.creative_url || (ad?.creative_urls && ad.creative_urls.length > 0);
  const imageUrl = ad?.creative_url || (ad?.creative_urls?.[0]);

  const banner = (
    <div className={variant === "sticky" ? "fixed bottom-[57px] left-0 right-0 z-40 px-0" : ""}>
      <AnimatePresence mode="wait">
        ...
        >
          {/* Ad label */}
          <span className="absolute top-1 right-1 text-[9px] font-medium text-muted-foreground/60 bg-background/50 px-1.5 py-0.5 rounded">Ad</span>

          {hasImage ? (
            <img src={imageUrl!} alt={ad?.title || "Ad"} className="h-14 w-14 rounded-lg object-cover shrink-0" />
          ) : (
          ...
          <div className="flex-1 min-w-0">
            <p className={`font-bold text-sm ${variant === "sticky" ? "text-primary-foreground" : "text-foreground"}`}>
              {ad?.title || "Sponsored"}
            </p>
            <p className={`text-xs line-clamp-1 ${variant === "sticky" ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
          ...
          <Button
            ...
            className={`rounded-lg text-xs shrink-0 ${
              variant === "sticky" ? "bg-background text-foreground hover:bg-background/90" : ""
            }`}
            onClick={(e) => {
              e.stopPropagation();
              handleClick();
      ...
      {activeAds.length > 1 && (
        <div className={`flex justify-center gap-1 mt-1.5 ${variant === "sticky" ? "bg-primary/80 pb-1.5" : ""}`}>
          {activeAds.map((_, i) => (
            <div
              key={i}
              className={`h-1 w-1 rounded-full transition-colors ${
                i === index
                  ? variant === "sticky" ? "bg-primary-foreground" : "bg-primary"
                  : variant === "sticky" ? "bg-primary-foreground/30" : "bg-muted-foreground/30"
              }`}
            />
          ))}
        </div>
        ...
        {expanded && ad && (
          ...
          >
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-6 text-center overflow-hidden">
              {hasImage ? (
                <img src={imageUrl!} alt={ad.title} className="w-full max-h-[50vh] rounded-2xl object-cover mb-6" />
              ) : (
                <div className="w-full h-48 rounded-2xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center mb-6">
            ...
            <div className="flex flex-col gap-2 p-4 border-t border-border bg-card">
              {businessCard ? (
                <div className="grid grid-cols-2 gap-2">
                  <Button size="lg" className="gap-2 rounded-xl" onClick={() => handleCta("call")}>
                    <Phone className="h-4 w-4" /> Call Now
                  </Button>
                  <Button size="lg" variant="outline" className="gap-2 rounded-xl" onClick={() => handleCta("chat")}>
                    <MessageCircle className="h-4 w-4" /> Chat
                  </Button>
                  {(businessCard.maps_link || businessCard.latitude) && (
                    <Button size="lg" variant="outline" className="gap-2 rounded-xl" onClick={() => handleCta("directions")}>
                      <MapPin className="h-4 w-4" /> Directions
                    </Button>
                  )}
                  <Button size="lg" variant="outline" className="gap-2 rounded-xl" onClick={() => handleCta("learn")}>
                    <ArrowRight className="h-4 w-4" /> View Business
                  </Button>
                </div>
              ) : (
                <Button size="lg" className="w-full gap-2 rounded-xl" onClick={() => setExpanded(false)}>
                  {ad.cta || "Learn More"}
                </Button>
              )}
            </div>
